...
import com.rts.ccp.dto.ResponsesDTO;
...
import com.rts.ccp.repository.ResponseRepo;
...
public class ResponseService {
	...
	public synchronized boolean insert(ResponsesDTO responsesdto) {
		
		System.out.println(responsesdto);
		if((responsesdto.getOptionType()).equalsIgnoreCase("Radio")) {
			
		SurveyOptions options = opRepo.findById(responsesdto.getOptionId()).get();
		...
		rsRepo.save(responses);
			...
			while(opIt.hasNext()) {
				Long lg=opIt.next();
                SurveyOptions opt=opRepo.findById(lg).get();
