...
import com.rts.ccp.dto.SurveyCommentDTO;
import com.rts.ccp.repository.SurveyCommentRepo;
...
public class SurveyCommentService {
	...
	public synchronized boolean insertComment(SurveyCommentDTO cmt) {
		
		Survey survey = surveyRepo.findById(cmt.getSurveyId()).get();
		User user = userRepo.findById(cmt.getUserId()).get();
		
		SurveyComment comment = new SurveyComment();
...
//	}
	...
	public List<SurveyComment> getAllCommentDetails() {
		Iterator<SurveyComment> it = commentRepo.findAll().iterator();
		ArrayList<SurveyComment> list = new ArrayList<>();
		while (it.hasNext()) {
			list.add(it.next());
		}
		return list;
