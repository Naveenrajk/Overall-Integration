...
public class PasswordResetService {
	...
	public String updatePassword(String email, String oldPassword, String newPassword, String confirmPassword) {
		...
		if (login != null) {
			...
			if (login.getPassword().equals(passwordEncryption.encrypt(oldPassword,lId))) {
				if (newPassword.equals(confirmPassword)) {
					// Additional functionality
					PasswordManager passwordManager = passwordManagerService.findByLoginId(lId);
					if (passwordManager != null) {
						if (passwordEncryption.encrypt(newPassword,lId).equals(passwordManager.getLastPassword1())
								||passwordEncryption.encrypt(newPassword,lId).equals(passwordManager.getLastPassword2())
								||passwordEncryption.encrypt(newPassword,lId).equals(passwordManager.getLastPassword3())) {
							...
							passwordManager.setLastPassword1(passwordEncryption.encrypt(newPassword,lId));
							passwordManagerService.saveOrUpdatePasswordManager(passwordManager);
							login.setPassword(passwordEncryption.encrypt(newPassword,lId));			
