...
public class OptionResponseController {
	...
	public ResponseEntity<OptionResponse> createVote(@PathVariable int pollId, @PathVariable int optionId,
			@PathVariable int userId) {

		// Ignore requests for favicon.ico

		if ("favicon.ico".equals(String.valueOf(pollId)) || "favicon.ico".equals(String.valueOf(optionId))
				|| "favicon.ico".equals(String.valueOf(userId))) {
	...
	@PutMapping("/{optionId}/{userId}/{pollId}")

	public void performResponseById(@PathVariable long optionId, @PathVariable long userId, @PathVariable long pollId) {

		optionResponseService.updateReponseById(optionId, userId,pollId);
...
//    
	...
	public Map<Integer, Integer> getTotalVoteCounts() {
	    List<OptionResponse> allVotes = optionResponseRepo.findAll(); // Assuming you have a method to get all responses
	    Map<Integer, Integer> totalVoteCounts = new HashMap<>();
	    
	    for (OptionResponse vote : allVotes) {
	        Integer optionId = vote.getOptionId();
	        totalVoteCounts.put(optionId, totalVoteCounts.getOrDefault(optionId, 0) + 1);
	    }
	    
	    return totalVoteCounts;
	}
	
	@GetMapping("/optionAllResponse")
	...
	public long getvoteCounts(@PathVariable ("pollId") long pollId) {
		return optionResponseService.getVoteCount(pollId);
	}
