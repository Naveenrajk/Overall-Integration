package com.rts.ccp.controller;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.CrossOrigin;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RestController;

import com.rts.ccp.bean.PollOption;
import com.rts.ccp.dto.PollOptionDTO;
import com.rts.ccp.dto.PollDTO;
import com.rts.ccp.service.OptionService;
import com.rts.ccp.service.PollService;

@CrossOrigin(origins = "http://localhost:4200")
@RestController
public class PollOptionController {

	@Autowired
	OptionService optionService;

	@Autowired
	PollService pollService;

	

	@PutMapping("/option")
	public String performOptionUpdate(@RequestBody PollOption option) {
		optionService.updateOption(option);
		return "Option Updated!";
	}

	@DeleteMapping("/option/{optionId}")
	public String performOptionDelete(@PathVariable("optionId") long optionId) {
		optionService.deleteOption(optionId);
		return "Option Deleted!";
	}

	@GetMapping("/option")
	public List<PollOption> viewAllOptionDetails() {
		return optionService.getAllOptionDetails();
	}

	@GetMapping("/options")
	public List<PollOptionDTO> OptionDetails() {
		return optionService.optionsDetails();
	}
	
	@GetMapping("/options/{pollId}")
	public  List<PollOption> getOptionDetails(@PathVariable("pollId") long pollId) {
		return optionService.optionValues(pollId);
	}
	
	@GetMapping("/optionValue/{pollId}")
	public List<PollOptionDTO> OptionDetails(@PathVariable("pollId") long pollId) {
		return optionService.optionValueDetails(pollId);
	}

}
