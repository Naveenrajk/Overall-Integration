...
login:Login;
  ...
  getdepartment(data: any) {
    this.department.regionList = data.region;
    // alert(this.department.regionList);
    this.departmentService.getParticularDepartment(this.department).subscribe(departments => this.depList = departments)
  }
  getproject(data: any) {
    this.project.departmentList = data.department;
    // alert(this.project.departmentList);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        setTimeout(function () {
          Swal.fire({
            title: 'Success',
            text: 'The user configured successfully!',
            icon: 'success',
            confirmButtonColor: '#3085d6',
  ...
  getEmployeeCount() {
    this.employeeservice.getEmployeeCount().subscribe(userCounts => this.userCount = userCounts);
