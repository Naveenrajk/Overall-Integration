...
//  region:Region;
 ...
 isHideComment=true;
   ...
   toggleLike(surveyId:number): void {
    //  if (this.liked) {
    //    this.count++;
    //  }
    //  else {
    ...
    //  this.showLike[surveyId] = !this.showLike[surveyId];
    //  if(this.isLike==true){
    //     this.count=0;
    //  }

    this.isShow[surveyId]=!this.isShow[surveyId];
    this.isLike[surveyId]=!this.isLike[surveyId];
   }
  ...
  getReplies(commentId: string): SurveyCommentInterface[] {
   ...
   getAllSurvey(){
    this.user.userId = this.userId;
    this.template.getAllSurvey(this.user).subscribe(all=>{
      this.allSurvey=all;
   ...
   reactionInsert(surveyId: number,value:boolean) {
     console.log(surveyId)
     this.rn.surveyId = surveyId;
     this.rn.userId = this.userId;
     console.log(this.rn);
    ...
    //  this.TotalnumberofLikes[surveyId]
     if (value==true) {
       ...
       console.log("Delete");
       this.results = this.template.reactionDeleteByUser(this.rn);
       location.reload();
     } else {
   ...
   getSurveyId(data:any){
     console.log(data);
     sessionStorage.setItem("surveyId",data);
   }
