...
export class TemplateService {
  ...
  deleteSurvey(survey: Survey): Observable<any> {
    return this.http.delete(this.url + "/survey/" + survey.surveyId);
  }
  ...
  reactionDeleteByUser(reaction: Reaction): string {
   this.http.post(this.url + '/reactionDelete', reaction).subscribe();
    return "Reaction Deleted!";
  }
  ...
  getAllSurvey(user: User){
    return this.http.get<Survey[]>(this.url + "/surveyv/"+user.userId);
  }
